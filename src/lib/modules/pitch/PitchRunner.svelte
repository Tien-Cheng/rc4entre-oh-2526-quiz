...
<div class="mx-auto flex min-h-dvh w-full max-w-5xl items-center px-5 py-7">
	<div class="glow-card w-full rounded-3xl p-6 md:p-8">
		<p class="text-xs uppercase tracking-[0.2em] opacity-70">Pitch Challenge</p>
		...
		{:else}
			<div class="mt-5 grid gap-4 md:grid-cols-2">
				<div class="rounded-2xl border border-white/10 bg-white/5 p-4">
					<p class="text-xs uppercase tracking-[0.2em] opacity-70">Product</p>
					<p class="mt-2 text-xl font-bold">{prompt?.product}</p>
				</div>
				<div class="rounded-2xl border border-white/10 bg-white/5 p-4">
					<p class="text-xs uppercase tracking-[0.2em] opacity-70">Audience</p>
					<p class="mt-2 text-xl font-bold">{prompt?.audience}</p>
				</div>
			</div>

			<div class="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
				<div>
					<p class="text-xs uppercase tracking-[0.2em] opacity-70">Prep timer</p>
					<p class="font-['Kanit'] text-4xl font-extrabold text-[var(--brand-amber)]">{remainingSeconds}s</p>
				</div>
				<div class="space-x-2">
					<span class="badge badge-info badge-lg">Host bonus +{hostBonus}</span>
